...
public interface AccesoArtistaRepository extends JpaRepository<AccesoArtista, Long> {
    ...
    List<AccesoArtista> findAllAccesosByIdArtistaIdUsuarioIdPermiso(Long idArtista, Long idUsuario, Long idPermiso);

    @Query("select a from AccesoArtista a JOIN a.permiso p where a.usuario.id = ?1 and p.codigo !='PRESUPUESTOS_RESTRINGIDOS' and a.activo and a.artista.activo")
